...
public class Car extends Observable implements IVehicle, Observer {
	...
	private boolean crossingCar = false;
	...
	public void move() {
		...
		if (canMove) {
			// if crossing car and hit e w road
			if (crossingCar && currentY > 795 && currentY < 800) {
				// already crossed so head south
				if (currentX > 395 && currentX < 405) {
					currentY += speed;
					ivCar.setY(currentY);
				}
