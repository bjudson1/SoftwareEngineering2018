...
public class Road {
	...
	public void assignGate(CrossingGate gate) {
		gates.add(gate);
		if (carFactory != null)
			carFactory = new CarFactory(direction, new Point(startX - roadSize / 2, startY), gates, false); // allows
																											// additional
																											// gates.
																											// Needs
																											// fixing
	...
	public void addCarFactory(boolean makeCrossingCars) {
		if (carFactory == null) // We only allow one
			carFactory = new CarFactory(direction, new Point(startX - roadSize / 2, startY), gates, makeCrossingCars);
	}
